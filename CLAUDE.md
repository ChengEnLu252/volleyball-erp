...
1. **主機 / DB**：部署在哪？DB 用 Postgres？（建議）還是有指定？
...
### 起步技術選型（建議，Claude Code 可確認）
- DB：**Postgres**。dev 階段用本機 Docker 或免費 dev 實例（Neon/Supabase 之類），正式主機 P6 再定。
- Auth：**Auth.js（NextAuth）** email+password + 雜湊，snap 進既有 LoginGate / 權限矩陣。
