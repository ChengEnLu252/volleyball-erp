...
// ============================================================
// app/finance/refunds/page.tsx — 階段 10 新增：退費處理
// ============================================================
// 兩個分頁：
//   1. 待退費：getPendingRefunds 列出已取消場次的 paid Registration + 操作
//             [💰 退款] 開 RefundModal、[📋 放棄] 開 WaiveModal、衝突走 ConflictBanner
//   2. 退費歷史：getRefundHistory 列已決定的紀錄 + decision/venue 過濾
//
// 設計要點：
//   - 每張 row 自己一份 baseUpdatedAt snapshot + conflict state（沿用階段 9 transfers pattern）
//   - 退款 modal 預填淨額 + 上次付款方式，可改
//   - 樂觀鎖：mutation 帶 baseUpdatedAt，失敗則橫跨整列顯示 ConflictBanner
//   - 處理後 row 自動消失（refundDecision !== null → 不在 getPendingRefunds list 中）
// ============================================================
...
function fmtMethod(m: PaymentMethod | null): string {
  if (m === null) return '—'
    ...
    <div style={{ padding: 24 }}>
      ...
      <div className="rfd-wrap" style={{ paddingTop: 0 }}>
        ...
        <div style={{
          ...
          background: '#f5f4f0', borderRadius: 10, padding: 4, width: 'fit-content',
        }}>
          <TabButton active={tab === 'pending'} onClick={() => setTab('pending')}>
            💰 待退費 <span style={{
              marginLeft: 6, padding: '1px 7px', borderRadius: 999,
              background: pending.length > 0 ? '#e85d3a' : '#cbc9c2',
        ...
        {tab === 'pending' && (
          <PendingRefundsTab rows={pending} mounted={mounted} />
        )}
        {tab === 'history' && (
          <HistoryTab visible={visible} mounted={mounted} storeVersion={storeVersion} />
        )}
...
function PendingRefundsTab({ rows, mounted }: { rows: PendingRefundRow[]; mounted: boolean }) {
  if (!mounted) {
    return (
    ...
    <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0', overflow: 'hidden' }}>
      <div style={{ padding: '13px 20px', borderBottom: '1px solid #f0ede6', fontSize: 13, fontWeight: 600, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 11, color: '#888', fontWeight: 400 }}>
          總應退：<strong style={{ color: '#e85d3a' }}>${rows.reduce((s, r) => s + r.netPaid, 0).toLocaleString()}</strong>
        </span>
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        ...
        <tbody>
          {rows.map(r => (
            <PendingRow key={r.registrationId} row={r} />
          ))}
        </tbody>
      </table>
    </div>
...
// ============================================================
// 單筆待退費 row（含樂觀鎖 + 兩個 modal）
// ============================================================

function PendingRow({ row }: { key?: string | number; row: PendingRefundRow }) {
  // 階段 9 pattern：每張 row 自己 baseUpdatedAt snapshot + conflict
  const [baseUpdatedAt, setBaseUpdatedAt] = useState<string>(row.registrationUpdatedAt)
  const [conflict, setConflict] = useState<ConflictResult | null>(null)
  useEffect(() => {
    if (!conflict && row.registrationUpdatedAt !== baseUpdatedAt) {
      setBaseUpdatedAt(row.registrationUpdatedAt)
    }
  ...
  return (
    <>
      <tr style={{ borderTop: '1px solid #f5f4f0', borderBottom: conflict ? 'none' : '1px solid transparent' }}>
        <td style={{ padding: '12px 10px', verticalAlign: 'middle' }}>
          <div style={{ fontWeight: 600 }}>{row.customerName}</div>
          {row.customerPhone && (
            <div style={{ fontSize: 11, color: '#aaa', marginTop: 2 }}>{row.customerPhone}</div>
          )}
        ...
        <td style={{ padding: '12px 10px', verticalAlign: 'middle' }}>
          ...
          <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>
            {row.sessionDate} {row.sessionStartTime}
          </div>
        </td>
        <td style={{ padding: '12px 10px', verticalAlign: 'middle', fontSize: 11, color: '#888' }}>
          ...
          {row.sessionCancelDetail && (
            <div style={{ marginTop: 3, color: '#aaa', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={row.sessionCancelDetail}>
              {row.sessionCancelDetail}
            </div>
          )}
        </td>
        ...
        <td style={{ padding: '12px 10px', verticalAlign: 'middle', fontSize: 12, color: '#666' }}>
          {fmtMethod(row.lastPaymentMethod)}
        </td>
        <td style={{ padding: '12px 10px', verticalAlign: 'middle', textAlign: 'right' }}>
          <button
            type="button"
          ...
          <button
            ...
            style={{
              padding: '6px 12px', borderRadius: 6,
              border: '1px solid #cbc9c2', background: '#fff',
      ...
      {conflict && (
        <tr>
          <td colSpan={6} style={{ padding: '8px 10px', background: '#fff', borderBottom: '1px solid #f5f4f0' }}>
      ...
      {refundOpen && (
        <RefundModal
          ...
          onConflict={c => { setRefundOpen(false); setConflict(c) }}
        />
      )}
      {waiveOpen && (
        <WaiveModal
          ...
          onConflict={c => { setWaiveOpen(false); setConflict(c) }}
        />
      )}
    </>
  )
...
function RefundModal({
  row, baseUpdatedAt, onClose, onConflict,
}: {
  ...
  function handleSubmit() {
    ...
    if (!Number.isFinite(num) || num <= 0) {
      setErrorMsg('退款金額必須大於 0')
      return
    }
    if (num > row.netPaid) {
      setErrorMsg(`不能超過應退 $${row.netPaid}`)
      return
    }
    setSubmitting(true)
  ...
  return (
    <Modal title={`退款給 ${row.customerName}`} onClose={onClose} accent="#166534">
      <div style={{ marginBottom: 14, padding: '10px 12px', background: '#f7fdf9', borderRadius: 8, fontSize: 12 }}>
        <div style={{ color: '#555' }}>
          場次：<strong>{row.venueName}</strong> · {row.sessionDate} {row.sessionStartTime}
        </div>
        <div style={{ color: '#555', marginTop: 4 }}>
          客戶淨付額：<strong style={{ color: '#e85d3a' }}>${row.netPaid.toLocaleString()}</strong>
          <span style={{ color: '#aaa', marginLeft: 8 }}>（原 {fmtMethod(row.lastPaymentMethod)}）</span>
        </div>
      </div>
      ...
      <label style={{ display: 'block', marginBottom: 14 }}>
        ...
        <input
          ...
          style={{
            ...
            border: '1px solid #cbc9c2', fontSize: 14, fontWeight: 600,
            boxSizing: 'border-box',
          }}
        />
        <div style={{ fontSize: 11, color: '#aaa', marginTop: 4 }}>
          可改：例如手續費自理 / 部分退費等情境。最高 ${row.netPaid}
        </div>
      </label>
          ...
          {(['cash', 'transfer', 'online'] as PaymentMethod[]).map(m => (
            <button
              ...
              style={{
                ...
                border: method === m ? '2px solid #166534' : '1px solid #cbc9c2',
                background: method === m ? '#dcfce7' : '#fff',
                color: method === m ? '#166534' : '#555',
                fontSize: 12, fontWeight: 600, cursor: 'pointer',
              }}
            >
      ...
      <label style={{ display: 'block', marginBottom: 14 }}>
        ...
        <input
          ...
          placeholder="例：已 LINE 通知 / 處理方式..."
          style={{
            width: '100%', padding: '9px 12px', borderRadius: 8,
            border: '1px solid #cbc9c2', fontSize: 13,
            boxSizing: 'border-box',
          }}
        />
      </label>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        ...
        <button
          ...
          style={{
            ...
            background: '#166534', color: '#fff', fontSize: 13, fontWeight: 600,
...
function WaiveModal({
  ...
  function handleSubmit() {
    if (!reason.trim()) {
      setErrorMsg('請填寫放棄退費的原因')
      return
    }
    setSubmitting(true)
    const result = waiveRefund({
      registrationId: row.registrationId,
      reason: reason.trim(),
      baseUpdatedAt,
    })
  ...
  return (
    <Modal title={`放棄 ${row.customerName} 的退費`} onClose={onClose} accent="#a16207">
      <div style={{ marginBottom: 14, padding: '10px 12px', background: '#fef9c3', borderRadius: 8, fontSize: 12, color: '#555' }}>
        ⚠ 此操作會將該筆退費標為「放棄」，<strong>不會建立任何金錢交易</strong>。
        通常用於：客戶同意改下週、信用券 / 點數抵償、金額太小不退等情境。
      </div>

      <div style={{ marginBottom: 14, fontSize: 12, color: '#555' }}>
        客戶淨付額 <strong style={{ color: '#e85d3a' }}>${row.netPaid.toLocaleString()}</strong> · 場次 {row.sessionDate} {row.sessionStartTime}
      </div>
      ...
      <label style={{ display: 'block', marginBottom: 14 }}>
        ...
        <textarea
          ...
          placeholder="例：客戶 LINE 確認改下週 / 已換信用券..."
          rows={3}
          style={{
            width: '100%', padding: '9px 12px', borderRadius: 8,
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        ...
        <button
          ...
          style={{
            padding: '8px 14px', borderRadius: 7, border: 'none',
            background: '#a16207', color: '#fff', fontSize: 13, fontWeight: 600,
            cursor: submitting ? 'not-allowed' : 'pointer',
            opacity: submitting ? 0.5 : 1,
          }}
        >
    ...
    >
      ...
      >
        <div style={{
          ...
          marginBottom: 16, paddingBottom: 12, borderBottom: `2px solid ${accent}`,
        }}>
...
function HistoryTab({
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const rows = useMemo<RefundHistoryRow[]>(() => {
    if (!mounted) return []
    const filter: RefundHistoryFilter = {}
    if (decisionFilter !== 'all') filter.decision = decisionFilter
    if (venueFilter !== 'all') filter.venueId = venueFilter
    const all = getRefundHistory(filter)
    // 額外套視角過濾（getRefundHistory 自己只看 filter.venueId 單一館；視角是多館 list）
    if (visible !== 'all' && venueFilter === 'all') {
      return all.filter(r => visible.includes(r.venueId))
    }
    return all
  }, [mounted, decisionFilter, venueFilter, visible, storeVersion])
  ...
  return (
    <div>
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0', padding: '14px 18px', marginBottom: 12, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {([
              ...
              <button
                ...
                style={{
                  ...
                  background: decisionFilter === key ? '#1a1917' : '#f5f4f0',
                  color: decisionFilter === key ? '#fff' : '#666',
                }}
              >
                {label}
              </button>
        ...
        <div>
          ...
          <select
            ...
            style={{ padding: '6px 12px', borderRadius: 7, border: '1px solid #cbc9c2', fontSize: 12, fontWeight: 500 }}
          >
        ...
        <div style={{ marginLeft: 'auto', fontSize: 12, color: '#666' }}>
          ...
          {rows.filter(r => r.decision === 'refunded').length > 0 && (
            <>
              {' '}· 已退 <strong style={{ color: '#166534' }}>${rows.filter(r => r.decision === 'refunded').reduce((s, r) => s + (r.refundedAmount ?? 0), 0).toLocaleString()}</strong>
            </>
          )}
        </div>
      </div>
      ...
      {!mounted ? (
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0', padding: 40, textAlign: 'center', color: '#888' }}>載入中…</div>
      ) : rows.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e6e0', padding: 40, textAlign: 'center', color: '#888' }}>
              ...
              {rows.map(r => (
                <tr key={r.registrationId} style={{ borderTop: '1px solid #f5f4f0' }}>
                  <td style={{ padding: '12px 10px', verticalAlign: 'top' }}>
                    {r.decision === 'refunded' ? (
                      <Badge color="green">✓ 已退款</Badge>
                    ) : (
                      <Badge color="gray">放棄</Badge>
                    )}
                  </td>
                  <td style={{ padding: '12px 10px', verticalAlign: 'top' }}>
                  ...
                  <td style={{ padding: '12px 10px', verticalAlign: 'top', textAlign: 'right' }}>
                    {r.refundedAmount !== null ? (
                      <span style={{ fontSize: 15, fontWeight: 700, color: '#166534' }}>${r.refundedAmount.toLocaleString()}</span>
                    ) : (
                  ...
                  <td style={{ padding: '12px 10px', verticalAlign: 'top', fontSize: 12, color: '#555', maxWidth: 280 }}>
                    {r.decision === 'refunded' ? (
                      <>
                        <div>{fmtMethod(r.refundMethod)}</div>
                        {r.refundNotes && <div style={{ color: '#888', marginTop: 2, fontSize: 11 }}>{r.refundNotes}</div>}
                      </>
                    ) : (
                      <div style={{ color: '#666' }}>{r.waiveReason ?? '—'}</div>
                    )}
                  </td>
                  <td style={{ padding: '12px 10px', verticalAlign: 'top', fontSize: 11, color: '#888' }}>
                    {r.decidedBy ?? '—'}
                    <div style={{ marginTop: 2 }}>{fmtTime(r.decidedAt)}</div>
                  </td>
                </tr>
              ))}
            </tbody>
