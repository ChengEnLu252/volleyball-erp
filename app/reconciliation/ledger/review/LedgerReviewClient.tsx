...
// 月記帳對帳（client）。資料自取自 server action（loadLedgerReviewAction）→ 不從 queries.ts
// 直接 import，避免 server-only 進 client bundle；可同時用於獨立路由與 bookkeeping 分頁 hub。
// 球館・月份切換 → 重新呼叫 action；逐日/逐月為本地切換；純渲染。

import { useEffect, useMemo, useState, useTransition } from 'react'
import { ReconHeader, StatCard, Panel, FilterButtons, Money } from '@/components/reconciliation/Common'
import type { LedgerCompareCell, LedgerReconResult, LedgerMonthSummary } from '@/data/ledger-core'
import { loadLedgerReviewAction } from '@/app/actions/ledger'
...
export default function LedgerReviewClient({ initialVenue, initialYm }: { initialVenue?: string; initialYm?: string }) {
  ...
  function load(venueId?: string, ym?: string) {
    startTransition(async () => {
      const res = await loadLedgerReviewAction({ venueId, ym })
      if (res.ok) setBundle({ venues: res.venues, venueId: res.venueId, ym: res.ym, recon: res.recon, summary: res.summary })
      else setBundle({ venues: [], venueId: '', ym: ym ?? currentYm(), recon: null, summary: null })
      setLoaded(true)
    })
  }
  ...
  if (venues.length === 0) {
    return (
      <div style={{ padding: 24 }}>
        ...
        <Panel><div style={{ padding: 24, color: '#888', fontSize: 13 }}>沒有可對帳的球館或尚未登入。</div></Panel>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div className="lr-wrap" style={{ paddingTop: 0, opacity: pending ? 0.6 : 1 }}>
        <ReconHeader
          title="月記帳對帳"
          subtitle="老闆視角：館長填的數字 vs 系統既有資料，差異自動標紅"
          backTo="/reconciliation"
          actions={
            <a href={`/reconciliation/ledger?venue=${venueId}&ym=${ym}`} style={{
        ...
        {recon && summary && (
          <>
            <div className="lr-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
              <StatCard label="已填天數" value={`${summary.filledDays} 天`} sub={`回報完畢 ${summary.reportedDays} 天`} accent="#2563eb" />
              <StatCard label="差異格數" value={`${recon.totals.flaggedCells} 格`} sub="場地/商品/退款逐日" intent={recon.totals.flaggedCells > 0 ? 'danger' : 'default'} accent="#e85d3a" />
              <StatCard label="場地費差異" value={fmtDiff(recon.totals.court)} sub={`館長 $${recon.totals.court.ledger.toLocaleString()}`} intent={isFlagged(recon.totals.court) ? 'danger' : 'default'} accent="#7c6af7" />
              <StatCard label="商品差異" value={fmtDiff(recon.totals.merch)} sub={`館長 $${recon.totals.merch.ledger.toLocaleString()}`} intent={isFlagged(recon.totals.merch) ? 'danger' : 'default'} accent="#f59e0b" />
            </div>
                        ...
                        {recon.daily.map((r) => {
                          ...
                          return (
                            <tr key={r.date} style={{ borderBottom: '1px solid #f7f6f2' }}>
                              ...
                              <td style={{ ...td, textAlign: 'center' }}>{r.hasLedger ? (r.reported ? '✓' : '–') : ''}</td>
                            </tr>
                          )
                        })}
                  ...
                  {recon.monthly.map((m) => (
                    <tr key={m.key} style={{ borderBottom: '1px solid #f5f4f0' }}>
                      ...
                      <td style={{ ...td, textAlign: 'right' }}>
                        {m.diff === null ? <span style={{ color: '#aaa' }}>—</span> : <Money value={m.diff} danger={m.diff !== 0} />}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div style={{ fontSize: 11, color: '#aaa', marginTop: 8 }}>＊誠實商店、季打月分攤的系統側待後續模組（P2.2e／月結整合）接上。</div>
            </Panel>
...
function isFlagged(c: LedgerCompareCell): boolean { return c.diff !== null && c.diff !== 0 }
...
function cellTriple(c: LedgerCompareCell, leftBorder: boolean) {
  ...
  return (
    <>
      <td style={{ ...td, textAlign: 'right', borderLeft: leftBorder ? '1px solid #f0ede6' : '1px solid #f7f6f2' }}>{c.ledger.toLocaleString()}</td>
...
function RO({ label, value, money = true, suffix = '' }: { label: string; value: number; money?: boolean; suffix?: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 16, fontWeight: 600, color: '#1a1917' }}>{money ? '$' : ''}{value.toLocaleString()}{suffix}</div>
    </div>
  )
}
