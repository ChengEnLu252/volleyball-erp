...
import {
  ReconHeader, StatCard, Panel, SeverityBadge, Money, VENUE_COLOR,
} from '@/components/reconciliation/Common'

const TYPE_ICON: Record<ReconciliationAnomalyType, string> = {
  session_shortfall:    '📅',
  rental_unpaid:        '🎫',
  gift_excess:          '📦',
...
export default function AnomaliesPage() {
  ...
  const anomalies = useMemo(() => {
    const all = getReconciliationAnomalies({
      venueId: venueId === 'all' ? undefined : venueId,
      type:    typeFilter === 'all' ? undefined : typeFilter,
    })
      .filter(a => severityFilter === 'all' || a.severity === severityFilter)
    if (visible === 'all') return all
    return all.filter(a => visible.includes(a.venueId))
  }, [typeFilter, venueId, severityFilter, visible])

  const allAnomalies = useMemo(() => {
    const full = getReconciliationAnomalies()
    if (visible === 'all') return full
    return full.filter(a => visible.includes(a.venueId))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [visible, storeVersion])
  ...
  const counts = useMemo(() => ({
    ...
    totalAmount: allAnomalies.reduce((s, a) => s + a.amount, 0),
  }), [allAnomalies])

  const filteredAmount = anomalies.reduce((s, a) => s + a.amount, 0)
    ...
    <div style={{ padding: 24 }}>
      ...
      <div className="an-wrap" style={{ paddingTop: 0 }}>
        ...
        <div className="an-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
          <StatCard
            label="異常總數"
            value={`${counts.total} 筆`}
            sub={`涉及 $${counts.totalAmount.toLocaleString()}`}
            accent="#7c6af7"
          />
        ...
        <div className="an-filter" style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
          <select
            value={typeFilter}
            onChange={e => setTypeFilter(e.target.value as ReconciliationAnomalyType | 'all')}
            style={selectStyle}
          >
            <option value="all">全部類型</option>
            {(Object.keys(ANOMALY_TYPE_LABEL) as ReconciliationAnomalyType[]).map(t => (
              <option key={t} value={t}>{ANOMALY_TYPE_LABEL[t]}</option>
            ))}
          </select>
          ...
          <select
            ...
            style={selectStyle}
          >
            {SEVERITY_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label === '全部' ? '全部嚴重度' : o.label}</option>)}
          </select>
          ...
          <span style={{ fontSize: 12, color: '#888', marginLeft: 'auto' }}>
            符合條件 {anomalies.length} 筆 · 涉及 ${filteredAmount.toLocaleString()}
          </span>
        </div>
          ...
          {anomalies.length === 0 ? (
            ...
            <div>
              {anomalies.map(a => (
                <Link
                  key={a.id}
                  href={linkFor(a.linkType, a.linkId)}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 12,
                    padding: '14px 0', borderBottom: '1px solid #f0ede6',
                    textDecoration: 'none', color: '#1a1917', flexWrap: 'wrap',
                  }}
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-start' }}>
                    ...
                    <span style={{
                      fontSize: 11, color: '#888', background: '#f5f4f0',
        ...
        <div style={{ fontSize: 11, color: '#aaa', textAlign: 'center', marginTop: 8 }}>
          異常條件：場次少收 ≥ $200・季租單 gap &gt; 0・商品贈送比例 &gt; 30%・無人場次自助回報筆數 &gt; Payment 筆數
        </div>
      </div>
    </div>
  )
